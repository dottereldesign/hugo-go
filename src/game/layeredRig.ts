...
export const HEAD_TURN_V3_FRAME_COUNT = 48;
...
export function getHeadTurnV3Pose(frameIndex: number): HeadTurnPose {
  const progress = normalizedFrame(frameIndex, HEAD_TURN_V3_FRAME_COUNT);
  return {
    yaw: -Math.PI / 2 + progress * Math.PI * 2,
    progress,
  };
